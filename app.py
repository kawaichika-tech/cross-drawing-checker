...
・ピンク線（アクセント壁）・黄色や緑の床着色・紫色のアクセント天井着色が「どの部屋に属するか」は、その色が引かれている領域の【内側】に書かれた室名で判断してください。隣の部屋や近くの部屋の室名と混同しないように注意してください。
・小さなスペースの部屋（トイレ・洗面室・納戸など）の室名は特に見落としやすいため、慎重に確認してください。小部屋に施された紫色のアクセント天井（斜め天井部分の紫着色など）を、隣接する大きな部屋（LDK・ホールなど）のものと取り違えないでください。
・「斜め天井」「アクセント天井」などの注記がある場合、その注記の引出線・矢印・配置位置がどの部屋を指しているかを必ず確認してください。注記の文言（例：「斜め天井部分もアクセント施工」）だけで部屋を推測せず、紫色が塗られている領域の【内側】の室名で帰属を確定してください。
...
・黄色 = クッションフロア（CF）のみ
・緑色 = フロアタイル（FT）のみ
この対応は絶対です。色と床材種類が入れ違いになっている場合は必ずエラーとして報告してください。
...
1. 黄色の着色（CF・クッションフロア）の照合:
   ...
   ③ 【重要】平面図で黄色に塗られているのに、表の床欄が「フロアタイル」になっている場合もエラーです。（黄色→CFが正しい。フロアタイルは緑色のはず）
...
3. 紫色の着色（アクセント天井）の照合:
   ...
   【手順 — 必ずこの順序で行うこと】
   STEP-A: 紫色の領域を1つずつ列挙してください。複数の紫色領域がある場合（例：キッチン上部とトイレなど別々の場所）、それぞれを独立した別の領域として扱ってください。
   STEP-B: 各紫色領域について、その領域の壁で囲まれた【内部】に書かれている室名を読み取ってください。領域の外側に書かれている室名（隣の部屋・大空間の総称・注記の文中の室名）は使わないでください。
   STEP-C: 注記（例：「斜め天井部分もアクセント施工」「アクセント天井」など）がある場合、その注記の引出線・矢印が指し示す先の部屋を特定し、その部屋の室名を使ってください。注記の中に「LDK」や「キッチン」などの文言が含まれていても、矢印の指す部屋を優先してください。
   STEP-D: 各紫色領域について、特定した室名で表の天井欄を照合してください。

   【重要な注意事項】
   ① 1つの紫色領域 = 1つの部屋です。1つの紫色領域を複数の部屋（例：「LDKの斜め天井部分」と「キッチン上部」の両方）に二重帰属させてエラー報告してはいけません。
   ② 「斜め天井」はトイレ・洗面所・階段下収納など、階段の下にある小部屋でもよく発生します。LDKのような大きな部屋の特徴と決めつけないでください。階段下の小部屋（特にトイレ）に「斜め天井部分もアクセント施工」の注記＋紫着色がある場合、その対象は小部屋自身であり、LDKではありません。
   ③ 表に天井品番の記載がない部屋（例：LDK）は、天井が基本クロス（標準仕上げ）であることを意味します。これは記載漏れではなくエラーでもありません。LDKに紫色の着色が【内側に】塗られていない限り、LDK天井の記載がないことをエラーにしないでください。
   ④ 注記の文言だけを根拠に「LDKの斜め天井がアクセントだが表にない」と判断してはいけません。必ず紫色の領域そのものと、その内部の室名を見てください。

4. 緑色の着色（フロアタイル）の照合:
   平面図で「緑色」に塗られている箇所と、表の「床（フロアタイル）」品番を照合してください。
   ① 表にフロアタイル品番があるのに図面が緑色に塗られていない場合もエラーです。
   ② 【重要】平面図で緑色に塗られているのに、表の床欄が「クッションフロア」になっている場合もエラーです。（緑色→フロアタイルが正しい。CFは黄色のはず）
